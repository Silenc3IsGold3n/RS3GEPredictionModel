...
def run(url,page,lockobject):
	...
	#cur.execute("create table if not exists item_Record_29_10_2017 (Id int, Type text ,Name text,Current_trend text,Current_price int, Today_trend text, Today_price text, Members bool)")
	for i in data:	
		#icon = data['icon']
		...
		#sqlq = "SELECT COUNT(1) FROM item_Record_29_10_2017 WHERE Id = ?"
		cur.execute(sqlq,(id,))
		if not cur.fetchone()[0]:
			print('Inserting item id = ' + str(id) +' into database.')
			sql = "INSERT INTO item_Record_"+date+ " VALUES (?,?,?,?,?,?,?,?)"
			#sql = "INSERT INTO item_Record_29_10_2017 VALUES (?,?,?,?,?,?,?,?)"
			#lockobject.acquire()
